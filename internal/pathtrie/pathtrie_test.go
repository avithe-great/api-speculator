...
func TestPathTrie_getNode(t *testing.T) {
	pt := New()
	if err := populateDummyPathsAndValue(pt,
		pathAndValue{path: "/api/{param1}/items", value: 1},
		pathAndValue{path: "/api/items", value: 2},
		pathAndValue{path: "/api/{param1}/{param2}", value: 3},
		pathAndValue{path: "/api/{param1}/cat", value: 4},
		pathAndValue{path: "/api/items/cat", value: 5},
	); err != nil {
		t.Error(err)
	}
...
func TestPathTrie_GetChildren(t *testing.T) {
	type fields struct {
		PathsAndValue []pathAndValue
	}
	tests := []struct {
		name   string
		fields fields
		...
		{
			name: "no paths returns no children",
			fields: fields{
				PathsAndValue: []pathAndValue{},
			},
			want: []string{},
		...
		{
			name: "single path returns path and prefix",
			fields: fields{
				PathsAndValue: []pathAndValue{
					{path: "/api/items", value: 1},
				},
			},
			want: []string{
				"/api",
		...
		{
			name: "multiple paths returns all children with prefix",
			fields: fields{
				PathsAndValue: []pathAndValue{
					{path: "/api/items", value: 2},
					{path: "/api/items/cat", value: 5},
					{path: "/api/{param1}/items", value: 1},
					{path: "/api/{param1}/cat", value: 4},
					{path: "/api/{param1}/{param2}", value: 3},
				},
			},
			want: []string{
				"/api",
		...
		{
			name: "duplicate paths returns unique children",
			fields: fields{
				PathsAndValue: []pathAndValue{
					{path: "/api/items", value: 1},
					{path: "/api/items", value: 2},
		...
		{
			name: "root path only returns no path",
			fields: fields{
				PathsAndValue: []pathAndValue{
					{path: "/", value: 1},
				},
		...
		{
			name: "multiple paths with trailing slashes returns correct children ",
			fields: fields{
				PathsAndValue: []pathAndValue{
					{path: "/api/items/", value: 1},
					{path: "/api/items/cat/", value: 1},
					{path: "/api/", value: 1},
				},
			},
			want: []string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pt := New()
			for _, pathNdValue := range tt.fields.PathsAndValue {
				pt.Insert(pathNdValue.path, pathNdValue.value)
			}

			got := pt.GetChildren()
			if len(got) != len(tt.want) {
				t.Errorf("GetChildren() returned %d items, want %d", len(got), len(tt.want))
			}

			for _, child := range got {
				if !slices.Contains(tt.want, child) {
					t.Errorf("GetChildren() = %v, want %v", child, tt.want)
				}
			}
		})
	}
...
func marshal(trie PathToTrieNode) any {
	trieBytes, _ := json.Marshal(trie)
	return string(trieBytes)
}

func populateDummyPathsAndValue(trie PathTrie, pathAndValues ...pathAndValue) error {
	for _, pathAndVal := range pathAndValues {
		if !trie.Insert(pathAndVal.path, pathAndVal.value) {
			return fmt.Errorf("insertion failed for path %s and value: %d", pathAndVal.path, pathAndVal.value)
		}
	}
	return nil
}
